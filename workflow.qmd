# Project Workflow {#sec-workflow}

Most of my academic work uses R and Quarto as a
backbone. To summarize:

-   Create an R project (or python project) as the hub of the project.
...
changes to the analysis many months or years after you are gone. If you follow this
...
This is the problem that literate programing solves. This document is written in Quarto with 
embedded R chunks. 
For example, I can embed R code into this document. I can load a library, read a
dataset, and create a figure all at once.

```{r first_plot}
library(ggplot2)
# x-y plot of mpg vs displacement
...
```{r second_plot}
# statistical density functions of mpg by class
...
## Project Folders {#sec-r-projects}

The basis of every project --- a paper, thesis, or report --- should be a folder. 
This folder will contain data, scripts, code, and other files that you use to make the analysis. 
Some folders will differ from project to project, but a common structure is:
...
When you type a path in a script, it should reference its location 
*relative to the project folder*. So a script in `R/clean_data.R` might look like this:
...
shows how to implement this folder structure in an R project

I prefer using [VS Code](https://code.visualstudio.com) or [Positron](https://positron.posit.co) as the text editor/IDE. Others 
are fine as well.
:::
...
`data/` folder, and ignoring the downloaded file from Git to save space in the 
repository.
...
[Data Science Reporting with Quarto for Python](https://www.linkedin.com/learning/data-science-reporting-with-quarto-for-python/reporting-with-quarto-and-python?u=2153100)
...
The Box code must be the link for the direct download. Please 
[see the Box manual](https://support.box.com/hc/en-us/articles/360043697554-Configuring-Individual-Shared-Link-Settings)
